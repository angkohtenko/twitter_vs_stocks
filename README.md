...
For the Classification model, we decided to use *LogisticRegression* to predict the probability that a tweet of Elon Musk will have an impact on Tesla stock or the probability this event occurs. The advantage of this model, *LogisticRegression*, is that it can provide us with statistical information on the tweets' significance in relation to the change of stock price, but likewise, a limitation to this model is the assumption that there is *LogisticRegression* between the data and its prone to overfitting.
...
The overall accuracy of the model is **57%** which shows that the model is not able to accurately predict a link between a tweet and the change in Tesla stock price. Considering the number of variables that influence the change in stock price this is an expected result, as not all of Elon Musk's tweets are directly influencing investors and there are many other factors that influence the stock market.

However, it is to be noted that when observing the *results_test* data frame, we can see that the model is able to correctly identify the most **positive** tweets. This shows the model is able to make good **NLP** deduction calls on tokens and is able to learn from the data.
